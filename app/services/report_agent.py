...
This is the strongest retrieval tool for deep analysis. It:
1. Breaks a question into sub-questions automatically
2. Retrieves information from multiple dimensions of the simulation graph
3. Combines semantic facts, entity analysis, and relationship tracing
4. Returns the richest evidence set for report writing

Use it when:
- you need deep analysis of a topic
- you need to understand multiple aspects of an event
- you need strong supporting material for a report section
...
[Panorama Search - Full Context View]
This tool gives a broad view of the simulation outcome, especially useful for tracking evolution over time. It:
1. Retrieves all relevant nodes and relationships
2. Separates currently active facts from historical / expired facts
3. Helps you understand how the event evolved

Use it when:
- you need the full development path of an event
- you need to compare different stages of opinion change
- you need comprehensive entity and relationship coverage
...
A lightweight retrieval tool for simple, direct lookups.

Use it when:
- you need to quickly find a specific detail
- you need to verify a fact
- you need a simple retrieval pass
...
Calls the live OASIS interview API to interview running simulated agents.
This is not an LLM roleplay. It retrieves the original answers from the active simulation.
By default, it interviews both Twitter and Reddit agents for broader perspective.

Workflow:
1. Load all available agent profiles
...
5. Combine the results into a multi-perspective view

Use it when:
- you need perspectives from different roles
- you need to collect multiple opinions and positions
- you need authentic answers from the simulated agents
- you want vivid, quote-ready interview material

Returns:
- interviewed agent identity information
- interview answers from Twitter and Reddit
- key quotes
- summary and comparison of viewpoints
...
PLAN_SYSTEM_PROMPT = """\
You are an expert writer of future-forecast reports with a god's-eye view of the simulation world. You can observe every simulated agent's behavior, speech, and interaction.

Core idea:
The simulation world contains a specific injected scenario. Its evolution represents a forecast of what may happen in the future. You are not analyzing present-day facts. You are reading a future rehearsal.

Your task:
Write a future-forecast report outline that answers:
1. What happened in the simulated future under the given conditions?
2. How did different groups of agents react and act?
3. What future trends, risks, and signals emerged?

Positioning:
- This is a future-forecast report built from simulation results.
- Focus on predicted outcomes: event direction, group reactions, emergent patterns, and risks.
- Agent behavior inside the simulation is evidence for projected future behavior.
- Do not write it as a report about current real-world conditions.
...
Return a JSON report outline in this format:
{
    "title": "Report title",
...
1. What state does the future settle into under these conditions?
2. How do different groups of agents react and act?
3. What future trends are worth highlighting?

Design the most suitable report structure based on the forecast.

Reminder: the report must have 2 to 5 sections, and it should stay concise and focused on the core forecast findings."""
...
Core idea:
The simulation world is a rehearsal of the future. The agents' behavior and interactions are evidence for projected future human behavior under the injected scenario.

Your job:
- explain what happened in the simulated future
- show how different groups reacted and acted
- surface notable future trends, risks, and opportunities

Mandatory rules:
1. You must observe the simulation through tools before writing.
2. Every section must use 3-5 tool calls.
3. All claims must come from simulation evidence, not your own general knowledge.
4. When useful, quote agent language or simulation evidence directly.
5. If a tool result is in another language, translate it into English while preserving meaning.
...
- Use paragraphs, lists, quotes, and **bold text** for structure.
- Keep quotes in their own paragraph.

Available retrieval tools:
{tools_description}

Suggested tool mix:
- `insight_forge` for deep multi-angle analysis
- `panorama_search` for timeline and whole-picture context
- `quick_search` for validating a specific point
- `interview_agents` for first-person reactions from simulated roles

Reply pattern:
- Either call exactly one tool using:
<tool_call>
{{"name": "tool_name", "parameters": {{"param": "value"}}}}
</tool_call>
- Or, if you have enough evidence, output the final section starting with `Final Answer:`

Never do both in the same reply.
Never fabricate tool results.
The section must read as a forecast based on the simulation outcome, not as a generic summary of the real world."""

SECTION_USER_PROMPT_TEMPLATE = """\
Completed sections so far. Read them carefully and avoid repetition:
{previous_content}
...
2. You must call tools before writing the section.
3. Mix different tools instead of relying on only one.
4. The report content must come from retrieved simulation evidence, not your own background knowledge.

Formatting rules:
- Do not write any Markdown headings.
- Do not start with "{section_title}".
- The system adds the section title automatically.
- Write body prose directly and use **bold text** instead of headings.

Start by thinking about what information this section needs, call tools to gather evidence, and then output `Final Answer:` with body text only."""
...
- If you have enough evidence, output the section starting with "Final Answer:" and quote the evidence above where relevant.
- If you need more evidence, call one more tool.
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "You have only used {tool_calls_count} tools, but at least {min_tool_calls} are required. "
    "Call another tool to gather more simulation evidence before writing Final Answer.{unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "You have used only {tool_calls_count} tools; at least {min_tool_calls} are required. "
    "Call another tool to retrieve simulation data.{unused_hint}"
)

REACT_TOOL_LIMIT_MSG = (
    "You have reached the tool-call limit ({tool_calls_count}/{max_tool_calls}) and cannot call more tools. "
    'Write the section now, starting with "Final Answer:".'
)

REACT_UNUSED_TOOLS_HINT = "\nHint: you have not used these tools yet: {unused_list}. Consider using them for broader evidence."
...
2. Answer directly and avoid long meta-reasoning.
3. Only call tools when the report is not sufficient.
4. Keep the answer concise, clear, and structured.

Available tools (only when needed, maximum 1-2 calls):
{tools_description}
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    ...
                    "report_context": "Optional section context to help generate more precise sub-questions"
                }
            },
            "panorama_search": {
            ...
            "quick_search": {
                ...
                "parameters": {
                    ...
                    "limit": "Optional result count limit (default: 10)"
                }
            },
            "interview_agents": {
        ...
        for iteration in range(max_iterations):
            ...
            if has_final_answer:
                ...
                if tool_calls_count < min_tool_calls:
                    ...
                    unused_hint = f" Unused tools you can still try: {', '.join(unused_tools)}." if unused_tools else ""
                    messages.append({
                        "role": "user",
