...
if os.path.exists(_env_file):
    ...
    print(f"Environment config not found: {_env_file}")
...
try:
    ...
    print("Please install the dependencies first: pip install oasis-ai camel-ai")
    sys.exit(1)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after the simulation ends instead of entering wait-for-command mode'
    )
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Effective executed rounds: {args.max_rounds} (truncated)")
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering wait-for-command mode - environment stays alive")
    ...
    log_manager.info("=" * 60)
    log_manager.info("All tasks completed")
